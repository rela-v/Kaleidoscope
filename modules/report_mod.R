...
generate_html_report <- function(id, input_genes) {
  moduleServer(
    id,
    function(input, output, session) {
      ...
      copy_files <- function() {
        ...
        file.copy("www/ks-report.Rmd", temp_path, overwrite = TRUE)
        
        temp_path_logo <- file.path(tempdir(), "ks_new_logo.svg")
        file.copy("www/assets/images/ks_new_logo.svg", temp_path_logo, overwrite = TRUE)
      ...
      render_html_report <- function(input) {
        copy_files()
      ...
      # Reactively render HTML report
      observeEvent(input$dl, {
        render_html_report(input)
      })
    }
  )
}
